# Using AutoDoc with Ollama: Beginner's Guide

AutoDoc supports local AI models through Ollama, which is perfect if you want to:
...
The first command downloads the embedding model that AutoDoc uses to understand your code. The second downloads a small but capable language model for generating documentation.

## Step 3: Set Up AutoDoc

Clone the AutoDoc repository:
```bash
...
## Step 4: Use AutoDoc with Ollama
...
1. Build the docker image `docker build -f Dockerfile-ollama-local -t autodoc:ollama-local .`
2. Run the container:
   ```bash
   # For regular use
   docker run -p 3000:3000 -p 8001:8001 --name autodoc \
     -v ~/.adalflow:/root/.adalflow \
     -e OLLAMA_HOST=your_ollama_host \
     autodoc:ollama-local
...
When you select "Use Local Ollama", AutoDoc will:
...
When using Ollama with AutoDoc:
...
Using AutoDoc with Ollama gives you a completely local, private solution for code documentation. While it may not match the speed or quality of cloud-based solutions, it provides a free and privacy-focused alternative that works well for most projects.

Enjoy using AutoDoc with your local Ollama models!
